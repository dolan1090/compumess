 ...
 */
#[Package('inventory')]
class Migration1683877185EmployeePasswordRecoveryMailTemplate extends MigrationStep
{
    use UpdateMailTrait;

    public const TYPE = 'employee_password_recovery';
    ...
    private function updateMailTemplateContent(Connection $connection): void
    {
        $update = new MailUpdate(
            self::TYPE,
            (string) file_get_contents(__DIR__ . '/Fixtures/mails/employee_password_recovery/en-plain.html.twig'),
            (string) file_get_contents(__DIR__ . '/Fixtures/mails/employee_password_recovery/en-html.html.twig'),
            (string) file_get_contents(__DIR__ . '/Fixtures/mails/employee_password_recovery/de-plain.html.twig'),
            (string) file_get_contents(__DIR__ . '/Fixtures/mails/employee_password_recovery/de-html.html.twig')
        );
        ...
        if (!\in_array($defaultLanguageId, [$englishLanguageId, $germanLanguageId], true)) {
            $connection->insert(
                ...
                [
                    ...
                    'name' => 'Employee password recovery',
                    'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT),
                ]
            );
        }
        ...
        if ($germanLanguageId) {
            $connection->insert(
                ...
                [
                    ...
                    'name' => 'Mitarbeiter-Passwort-Wiederherstellung',
                    'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT),
                ]
            );
        }
