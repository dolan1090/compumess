...
namespace Swag\CustomizedProducts\Test\Template\Aggregate\TemplateOption;
...
class TemplateOptionValidatorTest extends TestCase
{
    ...
    private TemplateOptionValidator $optionTypeValidator;
    ...
    private OptionTypeCollection & MockObject $collection;
    ...
    public function testUpdateWithInvalidPercentageSurcharge(): void
    {
        ...
        try {
            ...
            static::assertStringContainsString(
                'The property "percentageSurcharge" should be set',
                $exception->getMessage()
            );
        }
    }

    public function testUpdateTypeWithInvalidRequiredAfterValue(): void
    {
        $templateId = Uuid::randomHex();
        $optionId = Uuid::randomHex();
        $productId = Uuid::randomHex();

        $templateData = $this->getColorSelectTemplateData(
            $templateId,
            $productId,
            $optionId,
            Uuid::randomHex(),
        ...
        /* @var EntityRepository|null $optionRepo */
        $optionRepo = $this->getContainer()->get('swag_customized_products_template_option.repository');

        static::assertNotNull($optionRepo);
        ...
        /** @var TemplateOptionEntity|null $afterUpdate */
        $afterUpdate = $optionRepo->search(new Criteria([$optionId]), $this->context->getContext())->first();

        static::assertNotNull($afterUpdate);
        static::assertSame(TextField::NAME, $afterUpdate->getType());
    }

    public function testUpdateTypeWithInvalidRequiredBeforeValue(): void
    {
        $templateId = Uuid::randomHex();
        $optionId = Uuid::randomHex();
        $productId = Uuid::randomHex();
        ...
        /** @var EntityRepository $optionRepo */
        $optionRepo = $this->getContainer()->get('swag_customized_products_template_option.repository');
    ...
    public function testUpdateTypeWithInvalidRequiredValue(): void
    {
        $templateId = Uuid::randomHex();
        $optionId = Uuid::randomHex();
        $productId = Uuid::randomHex();
    ...
    public function testUpdateValid(): void
    {
        ...
        $this->collection
            ->expects(static::once())
            ->method('getIterator')
            ->willReturn($this->getGenerator($instance));
    ...
    public function testInsertInvalidColorSelect(): void
    {
        ...
        $this->createTemplate($templateData, $templateId, false);
    ...
    public function testUpdateValidColorSelect(): void
    {
        ...
        $this->getBrowser()->request(
            'PATCH',
            '/api/swag-customized-products-template-option-value/' . $valueId,
            [],
            [],
            [],
            \json_encode([
    ...
    public function testUpdateInvalidColorSelect(): void
    {
        ...
        $templateData = $templateData = $this->getColorSelectTemplateData(
            $templateId,
            $productId,
            $optionId,
            $valueId,
    ...
    private function createTemplate(
        array $templateData,
        string $templateId,
        bool $assertTemplate = true
    ): ?TemplateEntity {
        $this->getBrowser()->request(
            'POST',
            '/api/swag-customized-products-template',
            [],
            [],
            [],
            \json_encode($templateData, \JSON_THROW_ON_ERROR)
        );

        return $assertTemplate ? $this->assertTemplate($templateId) : $this->getTemplate($templateId);
    }
