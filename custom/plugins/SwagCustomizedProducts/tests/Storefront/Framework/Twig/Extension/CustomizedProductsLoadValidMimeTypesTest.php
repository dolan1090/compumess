...
class CustomizedProductsLoadValidMimeTypesTest extends TestCase
{
    ...
    public function testGetValidMimeTypes(): void
    {
        ...
        static::assertSame(
            $imageValidator->getMimeTypes(),
            $this->filter->getValidMimeTypes($imageValidator->getType())
        );
        static::assertSame($fileValidator->getMimeTypes(), $this->filter->getValidMimeTypes($fileValidator->getType()));
        static::assertEmpty(
            $this->filter->getValidMimeTypes('invalid_type'),
            'Return value of invalid type is not empty.'
        );
    }

    public function testGetExcludedAllImageMimeTypes(): void
    {
        $imageValidator = new ImageMediaUploadValidator();
        $option = new TemplateOptionEntity();
        $option->setTypeProperties(
            ['excludedExtensions' => \array_keys($imageValidator->getExtensionsWithMimeTypes())]
        );

        static::assertEmpty($imageValidator->getMimeTypes($option));
    }

    public function testGetExcludedOneImageMimeTypes(): void
    {
        $imageValidator = new ImageMediaUploadValidator();
        $option = new TemplateOptionEntity();
        ...
        static::assertCount(
            \count($this->filter->getValidMimeTypes($imageValidator->getType())) - 1,
            $imageValidator->getMimeTypes($option)
        );
    }
}
