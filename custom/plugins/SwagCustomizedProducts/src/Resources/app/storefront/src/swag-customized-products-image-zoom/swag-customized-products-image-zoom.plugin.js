...
export default class SwagCustomizedProductsImageZoomPlugin extends Plugin {
    ...
    /**
     * Register an event listener to a global keyup event
     *
     * @returns {void}
     */
    registerKeyUpEventListener() {
        DomAccess.querySelector(document, 'body').addEventListener('keyup', this.onEscapePressed.bind(this), { once: true });
    }
    ...
    onCloseClicked(event) {
        ...
        Backdrop.remove();
        DomAccess.querySelector(document, 'body').removeChild(this.contentEl);
    }

    /**
     * Event listener handler which will be fired when the escape button has been pressed
     *
     * @event keydown
     * @fires onEscapeKeyUp
     * @param {Event} event
    ...
    onEscapePressed(event) {
        ...
        this.$emitter.publish('onEscapeKeyUp', {
            event,
        });
    ...
    spawnZoomModal() {
        ...
        Backdrop.create(() => {
            DomAccess.querySelector(document, 'body').appendChild(this.contentEl);
        });
