...
export default class SwagCustomizedProductsExclusionListValidation extends Plugin {
    ...
    static options = {
        ...
        scrollOffset: 80,
        prefixes: {
            optionId: 'swag-customized-products-option-id-',
            uploadElement: 'customized-products-dropzone-input-',
        },
    };
    ...
    init() {
        ...
        this.violationTemplate = this.violationHolderElement.querySelector('div');
        this.violationTemplateString = this.violationTemplate.cloneNode(true).children[0].outerHTML;
        ...
        this.exclusions = DomAccess.getDataAttribute(this.el, 'data-swag-exclusion-list-validation-options') ?? [];
        ...
        this.mergeExclusionWithElements(elements);
    ...
    onViolationHolderElementClick(event) {
        ...
        if (collapsePanel && !this.isStepByStepActive) {
            ...
            // See: https://getbootstrap.com/docs/5.0/components/collapse/#methods
            bootstrap.Collapse.getOrCreateInstance(collapsePanel).show();
        }
    ...
    /**
     * Maps the provided elements to the exclusions.
     * @param {{id: String, element: Node}[]} elements
     */
    mergeExclusionWithElements(elements) {
     ...
     * @deprecated tag:v5.0.0 - Will be removed in v5.0.0. Please call `mergeExclusionWithElements` instead.
     */
     ...
     */
    buildViolations() {
        let violations = this.exclusions.map((exclusion) => {
    ...
    extractIdFromElement(element) {
        ...
        return id.replace(this.options.prefixes.optionId, '').replace(this.options.prefixes.uploadElement, '');
    }
