...
use Swag\CustomizedProducts\Template\Aggregate\TemplateOptionPrice\TemplateOptionPriceEntity;
...
{
    final public const CUSTOMIZED_PRODUCTS_TEMPLATE_LINE_ITEM_TYPE = 'customized-products';
    final public const CUSTOMIZED_PRODUCTS_OPTION_LINE_ITEM_TYPE = 'customized-products-option';
    final public const CUSTOMIZED_PRODUCTS_OPTION_VALUE_LINE_ITEM_TYPE = 'option-values';
    final public const CUSTOMIZED_PRODUCTS_TEMPLATE_LINE_ITEM_CONFIGURATION_HASH = 'customized-products-configuration-hash';
    final public const CUSTOMIZED_PRODUCT_CONFIGURATION_KEY = 'customizedProductConfiguration';
    final public const FALLBACK_PRODUCT_NUMBER = '*';
    private const CUSTOMIZED_PRODUCTS_CART_DATA_KEY = 'swag-customized-products-template-';

    private ProductCollection $referenceProducts;

    public function __construct(
        private readonly EntityRepository $templateRepository,
        private readonly SalesChannelRepository $productRepository,
        private readonly TemplateConfigurationServiceInterface $templateConfigurationService
    ) {
        $this->referenceProducts = new ProductCollection();
    }
    ...
    public function collect(
        ...
        $customizedProductsTemplates = $this->fetchTemplates(\array_filter($templateIds), $data, $context);
        ...
        $this->loadReferenceProducts($customizedProductsLineItems, $context);
        ...
        foreach ($customizedProductsLineItems as $customizedProductsLineItem) {
            ...
            if (!($customizedProductsTemplate instanceof TemplateEntity)
                || !$this->customizedProductsAvailable($productLineItem, $customizedProductsTemplate)
            ) {
            ...
            $this->addConfigurationExtension($customizedProductsLineItem, $original, $context);
        }
    }

    private function loadReferenceProducts(
        ...
        foreach ($customizedProductsLineItems as $customizedProductsLineItem) {
            ...
            if ($productReferenceId) {
                $productReferenceIds[] = $productReferenceId;
            }
        }

        $criteria = new Criteria($productReferenceIds);
        $criteria->addAssociation(Migration1565933910TemplateProduct::PRODUCT_TEMPLATE_INHERITANCE_COLUMN);

        /** @var ProductCollection $referenceProducts */
        $referenceProducts = $this->productRepository->search($criteria, $salesChannelContext)->getEntities();
    ...
    private function enrichCustomizedProduct(
        ...
        if (!$customizedProductsLineItem->hasPayloadValue('productNumber')) {
            $customizedProductsLineItem->setPayloadValue('productNumber', self::FALLBACK_PRODUCT_NUMBER);
        }

        $this->enrichWithVariationSpecifications($customizedProductsLineItem);

        $optionLineItems = $customizedProductsLineItem->getChildren()->filterType(
            self::CUSTOMIZED_PRODUCTS_OPTION_LINE_ITEM_TYPE
        );

        $fallbackTaxId = $this->getFallbackTaxId($customizedProductsLineItem);
        ...
        foreach ($optionLineItems as $optionLineItem) {
            ...
            $optionLineItem->setPayloadValue('productNumber', self::FALLBACK_PRODUCT_NUMBER);
            if ($itemNumber !== null) {
            ...
            foreach ($optionValueLineItems as $optionValueLineItem) {
                ...
                $optionValueLineItem->setPayloadValue('productNumber', self::FALLBACK_PRODUCT_NUMBER);
                if ($itemNumber !== null) {
    ...
    private function getPrice(
        ...
        return new PercentagePriceDefinition($percentageSurcharge);
    }
    ...
    private function getAdvancedPrice(
        ...
        /** @var TemplateOptionPriceEntity $optionPrice */
        foreach ($prices as $optionPrice) {
            ...
            return new PercentagePriceDefinition($optionPrice->getPercentageSurcharge() ?? 0);
        }
    ...
    private function getFallbackTaxId(
        ...
        $product = $this->referenceProducts->get($productReferenceId);
    ...
    private function customizedProductsLineItemContainsAllRequiredOptions(
        ...
        $referencedOptionIds = \array_flip(\array_filter($optionLineItems->getReferenceIds()));
    ...
    private function addConfigurationExtension(LineItem $customizedProductsLineItem, Cart $original, SalesChannelContext $salesChannelContext): void
    {
        if ($original->getExtension(PriceDetailRoute::PRICE_DETAIL_CALCULATION_EXTENSION_KEY)) {
            return;
        }

        $configurationHash = $customizedProductsLineItem->getPayloadValue(self::CUSTOMIZED_PRODUCTS_TEMPLATE_LINE_ITEM_CONFIGURATION_HASH);
        if ($configurationHash === null) {
            return;
        }

        $templateConfiguration = $this->templateConfigurationService->getTemplateConfiguration($customizedProductsLineItem, $configurationHash, $salesChannelContext->getContext());
        if ($templateConfiguration === null) {
            return;
        }

        $customizedProductsLineItem->addExtension(self::CUSTOMIZED_PRODUCT_CONFIGURATION_KEY, $templateConfiguration);
    }

    private function enrichWithVariationSpecifications(LineItem $customizedProductsLineItem): void
    {
        /** @var LineItem $productLineItem */
        $productLineItem = $customizedProductsLineItem->getChildren()->filterType(
            LineItem::PRODUCT_LINE_ITEM_TYPE
        )->first();

        $customizedProductsLineItem->setPayloadValue(
            'options',
            $productLineItem->getPayloadValue('options')
        );
    }
}
