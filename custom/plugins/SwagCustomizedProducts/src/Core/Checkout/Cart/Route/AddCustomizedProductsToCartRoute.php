...
class AddCustomizedProductsToCartRoute extends AbstractAddCustomizedProductsToCartRoute
{
    public function __construct(
        private readonly CustomizedProductCartServiceInterface $customizedProductCartService,
     ...
     * )
     *
     * @Route("/store-api/customized-products/add-to-cart", name="store-api.customized-products.add-to-cart", methods={"POST"})
     */
    public function add(
        ...
        $oldHash = $customizedProductsData->getAlnum('oldHash');
        if ($oldHash !== '') {
            $templateLineItems = $cart->getLineItems()->filterType(CustomizedProductsCartDataCollector::CUSTOMIZED_PRODUCTS_TEMPLATE_LINE_ITEM_TYPE);
            ...
            foreach ($templateLineItems as $templateLineItem) {
                ...
                if ($configurationHash === null || $configurationHash !== $oldHash) {
                    continue;
                }

                $this->cartService->remove($cart, $templateLineItem->getId(), $salesChannelContext);
            }
        }
        ...
        $customizedProductsLineItem->addExtension(
            ...
            new AddCustomizedProductsToCartIdentifierExtension()
        );
