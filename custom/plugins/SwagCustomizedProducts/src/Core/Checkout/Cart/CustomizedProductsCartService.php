...
use Swag\CustomizedProducts\Storefront\Upload\Exception\SwagCustomizedProductsMaximumFileCountExceededException;
...
class CustomizedProductsCartService implements CustomizedProductCartServiceInterface
{
    ...
    public function loadOptionEntities(string $templateId, RequestDataBag $options, Context $context): TemplateOptionCollection
    {
        ...
        $optionIds = \array_filter($optionIds, static fn ($id) => Uuid::isValid($id));
    ...
    public function addOptions(
        ...
        foreach ($options as $optionId => $option) {
            ...
            if (($value === null || \strlen((string) $value) <= 0) && empty($values)) {
                continue;
            }
    ...
    /**
     * @throws SwagCustomizedProductsMaximumFileCountExceededException
     */
    private function enrichOptionLineItemWithMedia(
        ...
        if (isset($typeProperties['maxCount'])
            ...
            throw new SwagCustomizedProductsMaximumFileCountExceededException();
        }
