...
class TemplateOptionValidator implements EventSubscriberInterface
{
    public function __construct(
        ...
        private readonly OptionTypeCollection $typeCollection,
    ...
    {
        if ($event->getWriteContext()->hasState(VersionManager::MERGE_SCOPE)) {
            return;
        }
    ...
    public function postValidate(PostWriteValidationEvent $event): void
    {
        ...
        foreach ($event->getCommands() as $key => $command) {
            ...
            if (($command instanceof InsertCommand || $command instanceof UpdateCommand)
                ...
                if ($type !== ColorSelect::NAME && $type !== ImageSelect::NAME) {
                    continue;
                }
            ...
            if (isset($command->getPayload()['type_properties'])) {
                $typeProperties = \json_decode((string) $command->getPayload()['type_properties'], true);
    ...
    private function extractValue(array $payload): array
    {
        ...
        return \json_decode((string) $payload['type_properties'], true);
    }
    ...
    private function postValidateColorValue(string $commandKey, WriteCommand $command, ConstraintViolationList $violationList): void
    {
        ...
        if ($command instanceof InsertCommand) {
            ...
            if (isset($payload['value'])) {
                $value = \json_decode((string) $payload['value'], true);
            }
        }
    ...
    private function validateCheckboxIsNotRequired(
        ...
        if ($command instanceof UpdateCommand) {
            ...
            $beforeValue = (bool) $changeSet->getBefore('required');
            $afterValue = $changeSet->getAfter('required');

            if (
                ($beforeValue === false && $afterValue === null)
                || ($afterValue !== null && ((bool) $afterValue) === false)
            ) {
                return;
            }
        } else {
