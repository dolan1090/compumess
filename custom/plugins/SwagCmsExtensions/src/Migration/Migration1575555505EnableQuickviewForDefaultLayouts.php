...
class Migration1575555505EnableQuickviewForDefaultLayouts extends MigrationStep
{
    ...
    {
        $productBlocks = $connection->executeQuery(<<<SQL
            SELECT `id`
            FROM `cms_block`
            WHERE `type` IN ('product-listing', 'product-slider', 'product-three-column')
        SQL)->fetchAllAssociative();

        $quickviewSettingPresent = $connection->executeQuery(\sprintf(<<<SQL
            SELECT `cms_block_id` AS `id`
            FROM `%s`
        SQL, QuickviewDefinition::ENTITY_NAME))->fetchAllAssociative();
